...
- ~~**Mentions légales / politique de confidentialité Twouich**~~ — **fait le 19/09** : deux pages embarquées (`twouich_legal.html`, `twouich_privacy.html`) écrites sur des faits **vérifiés contre le code compilé** et reliées depuis la page À propos ; à embarquer avec la prochaine release. **Suite de chantier : neutraliser la télémétrie Firebase** (config upstream toujours active — voir politique, section « La vérité sur l'héritage »).
