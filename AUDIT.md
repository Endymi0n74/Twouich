...
~~**Surveillance de la dérive de format SSAI**~~ — **fait le 19/09/2026** : `patch/radar_ads.py` — la méthode du 18/09 devient une commande (token GQL anonyme → usher → variante, 5 cycles × 5 chaînes) avec **rejeu anti-fuite** — chaque playlist capturée repasse dans le miroir Python du sanitizer (`patch/tests/test_sanitizer.py`) ; verdict ❌ + ligne HLS brute si la sentinelle du miroir parle ou si un marqueur pub (`stitched-ad`/`Amazon`) survit au nettoyage. Harnais hors-réseau `patch/tests/test_radar.py` (10 vérifications, mordance prouvée par 2 mutations). **Cron quotidien en CI** (fait le 19/09/2026) : workflow `radar.yml` — harnais hors-réseau puis sondage `--quiet` sur 4 chaînes à forte charge, verdict 1 = run rouge + **issue dédoublonnée** ; les pannes réseau ne sont pas des alertes (l'absence de preuve n'alerte pas) ; `schedule` désactivé par GitHub après 60 jours d'inactivité du dépôt, relançable en dispatch (chaînes surchargeables). **Le §5 est désormais intégralement traité ou assumé : le résidu et la feuille de route vivent au §8 de `memory.md`** (assumés explicites + trois horizons).
* ~~**Emotes/badges/highlighter**~~ — **audité le 19/09/2026, aucun défaut fonctionnel, rien à patcher**. Code localisé dans le package obfusqué `A6/` : 6 endpoints exactement (7TV **v3** `7tv.io/v3/emote-sets/global` + `users/twitch/<id>`, BTTV `betterttv.net/3/cached/…`, FFZ `frankerfacez.com/v1/…`, replay `recent-messages.robotty.de`, CDN `jtvnw.net/emoticons/v2`) — tous conformes à la vérification d'obsolescence du 18/09. Preuve en direct (émulateur 153, chat `#niniste`) : les **5 chargeurs verts** (`FFZ/BTTV/7TV/Global emotes/SubEmote Done`), IRCv3 complet (tags/membership/commands ACK, JOIN, PONG), messages réellement rendus à l'écran ; le tag **`highlighted-message`** est traité (préfixe retiré, g.smali), le modèle de badges (`broadcaster/partner/vip/premium`, `userBadges` côté replay) est présent. Deux bruits **upstream bénins** observés et attribués : Glide « load for a destroyed activity » (chargement d'images vers une activité détruite, pattern connu) et `NumberFormatException` parseant le champ `status` ("Not Found") d'un **404 7TV par-chaîne** — erreur attrapée, loader terminé en « 7TV Done », comportement normal de l'API pour un compte sans 7TV. Limite documentaire : le rendu **pixel** des emotes/badges (images CDN) n'est pas traçable par logcat — la preuve de rendu repose sur le chat visible et les chargeurs verts.
